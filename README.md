...
- Translation is optional. You can use the browser built-in translator when available, or configure Baidu, Azure, Google Cloud, or an OpenAI-compatible LLM endpoint.

Select to Note is built for users who repeatedly collect small, high-signal web snippets into active Obsidian notes. It is not a full-page archiver; the product focus is fast selection, clean Markdown, local-first capture, and low setup friction.
...
- 翻译是可选功能。可优先尝试浏览器内置翻译，也可以配置百度、Azure、Google Cloud 或 OpenAI-compatible LLM。

Select to Note 面向高频把网页片段整理进当前 Obsidian 笔记的用户。它不是完整网页归档器，核心价值是快速选择、干净 Markdown、本地优先和低配置成本。
